...
class MembersOnlyArticlesController(Resource):
    
    def get(self):
        # Check if user is logged in
        user_id = session.get('user_id')
        if user_id:
            # User is logged in, retrieve members-only articles
    ...
    def show(self, id):
        ...
        user_id = session.get('user_id')
        if user_id:
            # User is logged in, retrieve the requested members-only article
...
api.add_resource(MembersOnlyArticlesController, '/members_only_articles', endpoint='member_index')
api.add_resource(MembersOnlyArticlesController, '/members_only_articles/<int:id>', endpoint='member_article')
